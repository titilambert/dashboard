...
import {UPWARDS, DOWNWARDS} from 'daemonsetdetail/sortedheader_controller';
...
/**
 * Controller for the daemonset details view.
 *
 * @final
 */
export default class DaemonSetDetailController {
  /**
   ...
   * @param {!backendApi.Events} daemonSetEvents
   * @ngInject
   */
  constructor(
      $mdMedia, $stateParams, $state, $resource, $log, daemonSetDetail,
      daemonSetEvents) {
    ...
    this.daemonSetEvents = daemonSetEvents;

    /** @export !Array<!backendApi.Event> */
    this.events = daemonSetEvents.events;
  ...
  handleEventFiltering() {
    this.events = this.filterByType(this.daemonSetEvents.events, this.eventType);
    this.events = this.filterBySource(this.events, this.eventSource);
  }

  /**
   * @param {!backendApi.DaemonSetPod} pod
   * @export
   */
  getPodLogsHref(pod) {
    return this.state_.href(
        logsStateName,
        new LogsStateParams(
            this.stateParams_.namespace, this.stateParams_.daemonSet, pod.name));
  }
